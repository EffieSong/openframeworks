...
void ofApp::setup(){
    ...
    //load font
    font.load("Montserrat-Medium.ttf",20);
...
// if(redApple.pos.x<490) redApple.stopRotate();
...
//--------------------------------------------------------------
void ofApp::draw(){
    if(ofGetElapsedTimef()<30) drawScene3(0);
    if(ofGetElapsedTimef()>30 && ofGetElapsedTimef()<60) drawScene2(30);
    if(ofGetElapsedTimef()>60 && ofGetElapsedTimef()<109) drawScene3(60);
    //drawfadeIn();
...
void ofApp::drawScene1(float time){
   // ofSetColor(ground);
    ...
    if(ofGetElapsedTimef()>6+time){
        chameleon.changeColorByElement(redApple, 0.02);//change color
    }
    if(ofGetElapsedTimef()>10+time && ofGetElapsedTimef()<18+time){//apple away
        chameleon.moveTo(ofVec3f(507,ofGetHeight()+elephant.getHeight()/2-530+285),1);
        redApple.rZ -= 1;
        redApple.pos -= ofVec3f(3,0);
    }
    if(ofGetElapsedTimef()>18+time){ //elephant appear
        chameleon.moveTo(ofVec3f(507,285),0.6);
        grayElephant.moveTo(ofVec3f(520,530), 0.6);
       // grayElephant.interpolateByPct(ofGetWidth()/2,ofGetHeight()+elephant.getHeight()/2,520,512,0.002f);
        ofSetColor(255);
         ofPushMatrix();
    ...
    if(ofGetElapsedTimef()>5+time){ //draw apple
        ofPushMatrix();
...
void ofApp::drawScene2(float time){
    ...
    if(ofGetElapsedTimef()> 20.000+time){
        emotion = 2;
        sad += 0.04;
        if(sad >10)sad=10;
    };
    drawfadeIn();
    if(ofGetElapsedTimef()>29.000+time){
        drawfadeOut();
    }
}
...
void ofApp::drawScene3(float time){
    ...
    if(ofGetElapsedTimef()> 5.000+time && ofGetElapsedTimef()<15+time){
        chameleon.moveTo(Leaf.pos-ofVec3f(20,5), 1.0);
        chameleon.changeColorByElement(Leaf,0.06);
    }
    if(ofGetElapsedTimef()>10.000+time && ofGetElapsedTimef()<15+time){
        sad -= 0.04;
        if(sad <0)sad=0;
    }
    
    if(ofGetElapsedTimef()>20+time){
       chameleon.changeColor(ofColor(225,180,29),0.04);
    }
    if(ofGetElapsedTimef()> 25.000+time){
        Leaf.rotate(0.2);
        branchPos += ofVec3f(0,-1,0);
        groundY -= 1;
        if(groundY < 350){
            groundY = 350;
        }
...
void ofApp::drawfadeOut(){
    ...
    alphaFadeOut += 2;
    if (alphaFadeOut>255)fbo_fadeOut.clear();
}
